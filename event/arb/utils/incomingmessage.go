...
func (msg *L1IncomingMessage) Serialize() ([]byte, error) {
	...
	if err := AddressTo256ToWriter(msg.Header.Poster, wr); err != nil {
		return nil, err
	}

	if err := Uint64ToWriter(msg.Header.BlockNumber, wr); err != nil {
		return nil, err
	}

	if err := Uint64ToWriter(msg.Header.Timestamp, wr); err != nil {
		return nil, err
	}
	...
	if err := HashToWriter(requestId, wr); err != nil {
		return nil, err
	}
	...
	if err := HashToWriter(l1BaseFeeHash, wr); err != nil {
		return nil, err
	}
...
func ParseIncomingL1Message(rd io.Reader, batchFetcher FallibleBatchFetcher) (*L1IncomingMessage, error) {
	...
	blockNumber, err := Uint64FromReader(rd)
	if err != nil {
		return nil, err
	}

	timestamp, err := Uint64FromReader(rd)
	if err != nil {
		return nil, err
	}
	...
	baseFeeL1, err := HashFromReader(rd)
	if err != nil {
		return nil, err
	}
...
func (msg *L1IncomingMessage) ParseInitMessage() (*ParsedInitMessage, error) {
	...
	if len(msg.L2msg) > 32 {
		...
		case 1:
			var err error
			basefee, err = Uint256FromReader(reader)
			if err != nil {
				return nil, err
			}
			fallthrough
