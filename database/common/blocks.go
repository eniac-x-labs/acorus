...
func (db *blocksDB) StoreBlockHeaders(chainId string, headers []ChainBlockHeader) error {
	result := db.gorm.Table(chainId+"_block_header").CreateInBatches(&headers, common2.BatchInsertSize)
	return result.Error
}
...
func (db *blocksDB) ChainBlockHeaderWithScope(scope func(*gorm.DB) *gorm.DB, chainId string) (*BlockHeader, error) {
	...
	result := db.gorm.Table(chainId + "_block_header").Scopes(scope).Take(&header)
	if result.Error != nil {
...
func (db *blocksDB) ChainLatestBlockHeader(chainId string) (*BlockHeader, error) {
	var header BlockHeader
	result := db.gorm.Table(chainId + "_block_header").Order("number DESC").Take(&header)
	if result.Error != nil {
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (db *blocksDB) LatestObservedEpochForChain(chainId string, fromL1Height *big.Int, maxL1Range uint64) (*BlockHeader, error) {
	...
	query := db.gorm.Table(chainId+"block_headers").Where("number >= ? AND number < ?", startBlockNumber, maxL1Range).Order("number DESC").Limit(1)
...
func (db *blocksDB) BlockTimeStampByNum(chainId string, blockNumber uint64) (int64, error) {
	...
	result := db.gorm.Table(chainId+"block_headers").Where("number = ?", blockNumber).Select("timestamp").Take(&blockTimestamp)
	if result.Error != nil {
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			return 0, nil
		}
