...
type DB struct {
	...
	AppChainUnStakeBatch  appchain.AppChainUnStakeBatchDB
	AppChainUnStakeSingle appchain.AppChainUnStakeSingleDB
}
...
func NewDB(ctx context.Context, dbConfig config.Database) (*DB, error) {
	...
	db := &DB{
		...
		AppChainUnStakeBatch:  appchain.NewAppChainUnStakeBatchDB(gorm),
		AppChainUnStakeSingle: appchain.NewAppChainUnStakeSingleDB(gorm),
	}
	return db, nil
}
